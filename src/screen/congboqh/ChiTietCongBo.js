...
const ChiTietCongBo =(props)=> {
    ...
    const dataChiTiet= props.route.params.data
     useEffect(()=>{
       
        console.log(dataChiTiet)
        ApiThuaDat.TangView(dataChiTiet.LstCongBo.ID,3);
    ...
    return (
        ...
        <HeaderMenu navigation={props.navigation}  titleHeader={"CHI TIẾT CÔNG BỐ"} />
        <Text style={[material.titleWhite,{textAlign:'center',flexWrap:'wrap',marginVertical:10,color:theme.colors.Primary}]}>{dataChiTiet.LstCongBo.TIEUDE}</Text>

       <View style={{ marginHorizontal: 15,margin:5}}>
          		<Text style={[material.caption,{color:theme.colors.Primary,textAlign:'right'}]}><FontAwesome5 name={"calendar"} />
 {" "+dataChiTiet.LstCongBo.CREATEDATE}</Text>
 <ScrollView style={{marginBottom:100}}>
 <RenderHTML
      contentWidth={theme.sizes.width-40}
      source={{html:dataChiTiet.LstCongBo.NOIDUNG}}
    />
 </ScrollView>
