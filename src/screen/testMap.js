...
const GetChiTiet=(ID)=>{
  ...
  const ChangeMap=()=>{
    console.log('123')
    let data = `{"styleHandle":"ChangeMap", "maxa":"26377", "mahuyen":"731"}`;
...
settenKVHC(item.ten)
  let data = `{"styleHandle":"ChangeMap", "maxa":"${item.maKvhc}", "mahuyen":"731"}`;
  webViewRef?.current?.postMessage(data);
...
const onMessage = (data) => {
  ...
  //console.log(temp.styleHandle)
  //console.log("ON MESSAGE: ", data);
  if(temp.styleHandle === "SendQH"){ 
    console.log("ON DATA: ", temp.data.QuyHoach);
    setdataQH(temp.data.QuyHoach)
    var dataToThuaAPI={
      SoTo:temp.data.SoTo,
      SoThua:temp.data.SoThua,
      DienTich:temp.data.DienTich

    } 
    refdataToThua.current=dataToThuaAPI
    setIsOpen(true)
  }
  // if(temp.styleHandle === "dataLogin"){ 
  //   dispatch(ResponseSignIn(temp.username, temp.password, temp.sotien, temp.token, temp.sdt, temp.hoten, temp.cmnd, temp.lathanhvien, temp.notification, temp.islogin, temp.email, temp.xacthucotp, temp.tokenotp, temp.codeotp));
  // }
  // else if(temp.styleHandle === "dataAuthSDT"){
  //   dispatch(ResponseAuthSDT(temp.token, temp.notification, temp.ketqua));
  // }
  // else if(temp.styleHandle === "dataReSendOTP"){
  //   dispatch(ResponseReSendOTP(temp.token, temp.codeotp, temp.ketqua));
  // }
  // else if(temp.styleHandle === "dataChangePassWord"){
  //   dispatch(ResponseChangePassWord(temp.ketqua, temp.notification));
  // }
  // else if(temp.styleHandle === "dataGiayChungNhan"){
  //   dispatch( (temp.ketqua, temp.notification, temp.data));
  // }
  // else if(temp.styleHandle === "dataLocation"){
  //   ChucNangDanDuong(temp.lat, temp.long);
  // }
  // else if(temp.styleHandle === "dataInfoThuaDat"){
  //   ShareThuaDat(temp.soto, temp.sothua, temp.dientich);
  // }
  // else if(temp.styleHandle === "dataResetPassword"){
  //   dispatch(ResponseResetPassword(temp.ketqua, temp.notification));
  // }
  // else if(temp.styleHandle === "dataUpdateUserInfo"){
  //   dispatch(ResponseUpdateUserInfo(temp.ketqua, temp.notification));
  // }
  // else if(temp.styleHandle === "dataCheckEmail"){
  //   dispatch(ResponseCheckEmail(temp.token, temp.ketqua, temp.notification));
  // }
  // else{
  //   Alert.alert("Không tìm thấy kiểu xử lý");
  // }
}
    return (
      ...
      <Header  scrollPoint={scrollY} navigation={props.navigation} ChonKVHC={()=>setIsOpenXa(true)} tenKVHC={tenKVHC} LayToThua={LayToThua} products={tempSearch}></Header>
          ...
          //source={{uri:'https://google.com'}}
           source={{uri:'http://192.169.6.97:9002'}}
          ...
          onError={(syntheticEvent) => {
            ...
            if(nativeEvent.code === -2){
              ...
              Alert.alert("Cảnh báo", "Thiết bị không có kết nối đến internet.");
            }
          }}       
        />
